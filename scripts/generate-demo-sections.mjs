 ...
 * config (`@knitui/plugins/next-plugin`). The `icons` package is EXCLUDED: it
 * ships ~6k generated icons, so instead of mirroring its story we render a
 * purpose-built, searchable browser — see `packages/demo/src/IconsSection.tsx`.
 *
...
/** Packages whose stories render in the gallery. Only `icons` is excluded (see above). */
const PACKAGES = ["components", "dates", "map", "carousel", "media", "sheet", "graphics"];
...
const body = entries
  .map(({ id, title, group, importPath, fullBleed, webglGallery, mediaGallery, needsGraphicsRuntime }) => {
