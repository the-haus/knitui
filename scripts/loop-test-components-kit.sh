#!/usr/bin/env bash
#
# loop-test-components-kit.sh
# ---------------------------
# Repeatedly invokes Claude Code (headless) to deepen the Jest + React Testing
# Library tests for EVERY component in @knitui/components until MOST OF EACH
# COMPONENT'S FEATURES ARE ACTUALLY COVERED — measured by per-component code
# coverage, not merely "a test file exists". When a test (or honest inspection
# against the Mantine public API it mirrors) shows behaviour that is broken,
# missing, or divergent, it FIXES the component source instead of weakening the
# test.
#
# It is the @knitui/components twin of scripts/loop-test-dates-kit.sh — identical
# engine (coverage-driven selection, lean rules file referenced by path, reflect →
# plan → execute → validate, diff-scoped strict-typing gate, live pretty-streamed
# output, resumable from disk), pointed at the base UI kit instead of the dates
# kit. Unlike scripts/loop-build-stories-tests-lean.sh (which considers a component
# "done" once a story + test file merely EXIST), this one keeps returning to thin
# tests until real coverage clears the bar.
#
# WHY COVERAGE DRIVES IT
#   Every component in @knitui/components already has a <Name>.test.tsx and the
#   Jest/RTL toolchain is configured (jest.config.js, jest.setup.ts, src/test-
#   utils.tsx — rendering the cross-platform Tamagui components on the WEB target
#   via react-native-web in jsdom). But many of those tests are thin (render the
#   default and stop). So "done" here is "the test drives the component's source
#   past COVERAGE_TARGET% statements". Each iteration the loop:
#     1. runs the suite WITH coverage (one run serves both the pass/fail gate and
#        the per-component coverage map),
#     2. picks the LOWEST-COVERED components (whose features are least exercised),
#     3. has Claude reflect → plan → deepen their tests (and fix the source when a
#        test exposes a real bug) → re-run the gate.
...
#   reflect → plan (.components-test-plan.md) → execute → validate. The gate =
#   `typecheck` AND the component test suite (run with --coverage) AND a diff-
#   scoped strict-typing check. Tests are the spec: a failure fails the iteration
#   and Claude must fix the ROOT CAUSE, preferring to fix the component over
#   weakening the test. When a coverage "cycle" reaches TARGET_FRACTION of
#   components at target, Claude reviews the whole suite and appends NEW
#   recommendations (.components-test-recommendations.md); the loop then works that
#   backlog and regenerates ideas each cycle until it converges or hits MAX_CYCLES
#   / MAX_ITERATIONS.
#
# Usage
#   scripts/loop-test-components-kit.sh                       # run with defaults (live output)
#   BATCH_SIZE=4 MAX_ITERATIONS=60 scripts/loop-test-components-kit.sh
#   COVERAGE_TARGET=85 scripts/loop-test-components-kit.sh     # raise the bar per component
#   ONLY="Button,Card,Tabs" scripts/loop-test-components-kit.sh   # subset
#   STREAM=text scripts/loop-test-components-kit.sh            # only final answers
#   DRY_RUN=1 scripts/loop-test-components-kit.sh              # show the work-list, write nothing
#
# Live + persisted output:
#   - Terminal shows readable activity in real time (STREAM=pretty, the default).
#   - .components-test-logs/iter-NNN-*.log        readable transcript per iteration
#   - .components-test-logs/iter-NNN-*.log.jsonl  full raw stream-json for that iteration
#
# NOTE: This runs Claude with --dangerously-skip-permissions so it can edit files
# (tests AND component source) and run the test suite unattended. Only run it in a
...
# The package we are covering with tests.
PKG_NAME="${PKG_NAME:-@knitui/components}"
PKG_DIR="${PKG_DIR:-packages/components}"

# References the tests assert against (read, never copied):
#   - Mantine CORE source = the PUBLIC prop API + behaviour each test pins down.
#     Defaults to the core checkout specifically (tmp/mantine also vendors dates,
#     carousel, …, so we point straight at core to avoid picking a sibling).
#   - Our @knitui/core source = the design-system foundation the kit is built on.
MANTINE_REF="${MANTINE_REF:-$REPO_DIR/tmp/mantine/core}"
CORE_REF="${CORE_REF:-$REPO_DIR/packages/core/src}"

PROGRESS_FILE="${PROGRESS_FILE:-$REPO_DIR/.components-test-progress}"
LOG_DIR="${LOG_DIR:-$REPO_DIR/.components-test-logs}"
JOURNAL="${JOURNAL:-$REPO_DIR/.components-test-journal.md}"           # running reflection log
PLAN_FILE="${PLAN_FILE:-$REPO_DIR/.components-test-plan.md}"          # current iteration plan
RECS_FILE="${RECS_FILE:-$REPO_DIR/.components-test-recommendations.md}"  # backlog of ideas
# Architecture/rules written ONCE at startup and referenced by every prompt.
RULES_FILE="${RULES_FILE:-$REPO_DIR/.components-test-rules.md}"
...
# Per-session turn cap (one session plans + deepens several tests + fixes sources +
...
# ---- Validation gate -------------------------------------------------------
# The gate = typecheck AND the component test suite (run WITH coverage so a single
# run gives both the pass/fail signal and the per-component coverage map) AND a
# diff-scoped strict-typing check.
TYPECHECK_CMD="${TYPECHECK_CMD:-pnpm --filter $PKG_NAME typecheck}"
# Jest runs in the package dir (via pnpm --filter exec), so the coverage globs and
# the coverage-summary.json output path are relative to packages/components.
TEST_CMD="${TEST_CMD:-pnpm --filter $PKG_NAME exec jest --coverage --coverageReporters=json-summary --coverageReporters=text-summary --collectCoverageFrom='src/**/*.{ts,tsx}' --collectCoverageFrom='!src/**/*.test.*' --collectCoverageFrom='!src/**/*.stories.*' --collectCoverageFrom='!src/**/index.{ts,tsx}'}"
# Where jest writes the machine-readable coverage map the loop parses.
COVERAGE_SUMMARY="${COVERAGE_SUMMARY:-$REPO_DIR/$PKG_DIR/coverage/coverage-summary.json}"
...
# Helpers
# ----------------------------------------------------------------------------
log()  { printf '\033[1;36m[comp-test]\033[0m %s\n' "$*"; }
warn() { printf '\033[1;33m[comp-test]\033[0m %s\n' "$*" >&2; }
die()  { printf '\033[1;31m[comp-test]\033[0m %s\n' "$*" >&2; exit 1; }
...
SRC_DIR="$REPO_DIR/$PKG_DIR/src"
[[ -d "$SRC_DIR" ]] || die "Components source dir not found: $SRC_DIR"
...
# ----------------------------------------------------------------------------
# One-time bootstrap: the shared render helper. @knitui/components already ships
# src/test-utils.tsx, so this is a no-op there; it only writes the helper (wraps
# trees in the kit's <Provider>) for a checkout where it is genuinely missing.
# ----------------------------------------------------------------------------
ensure_test_utils() {
...
# Components test-coverage rules (read fully before touching any file)

You are DEEPENING the Jest + React Testing Library TESTS of an existing
cross-platform UI kit, __PKG_NAME__ (dir __PKG_DIR__), whose components are built
on Tamagui via @knitui/core and whose PUBLIC API mirrors Mantine (prop names,
variants, sizes, compound parts, controlled/uncontrolled value, ref forwarding) —
colour being the one deliberate divergence (Tamagui `theme` prop + palette ramp,
not a Mantine `color` prop). The components ALREADY exist and most already have a
test file; your job is COVERAGE and CORRECTNESS, not growing the set or adding
stories.

THE GOAL IS COVERAGE, NOT PRESENCE:
  A component is "done" only when its __PKG_DIR__/src/<Name>/<Name>.test.tsx drives
  the component's source past __COVERAGE_TARGET__% statement coverage. A test that
  merely renders the default and stops is NOT done — the loop measures real
  per-component coverage and will keep returning to thin tests. Keep the test
  co-located in the component's own folder, next to <Name>.tsx.
...
jest.setup.ts):
  ...
  - Rendering/queries: React Testing Library (@testing-library/react) plus
    ...
    __PKG_DIR__/src/test-utils.tsx, which already wraps trees in the kit's
    `<Provider>`:
        import { render, screen, fireEvent } from "../test-utils";
    STUDY __PKG_DIR__/src/Button/Button.test.tsx — it is the canonical example;
    mirror its style. Do NOT add a new render/provider helper; reuse test-utils.
    Import the component under test from "./<Name>".
  - Components/Box/Text/Provider/Theme import from __PKG_NAME__ (or a sibling
    relative path within the package). NEVER import `tamagui` or `@tamagui/*` from
    a test file.

WHAT A THOROUGH TEST COVERS (assert real BEHAVIOUR, not snapshots):
  - Renders with defaults and exposes its accessibility role/label.
  - EVERY value of EVERY custom `variants` axis — each `variant`, each `size`, and
    any other axis declared in the `styled(…, { variants: { … } })` block — is
    rendered and asserted (an `it.each` over the value list is ideal), including
    boolean variants' true/false branches.
  - EVERY public prop in the component's `…Props` type has at least one assertion
    proving its documented effect (not merely that passing it doesn't crash);
    booleans test BOTH branches where behaviour differs (disabled, loading,
    readOnly, withBorder, …).
  - INTERACTION: controlled AND uncontrolled value flow where the component has
    value state; onChange/onClick/onPress/onOpenChange fire with the right
    argument(s); disabled/readOnly block interaction; keyboard a11y where Mantine
    specifies it.
  - EVERY compound sub-component (Card.Section, Menu.Item, Accordion.Item,
    Tabs.Tab, …) is mounted and its wiring to the parent (context, value,
    selection) asserted.
...
COMPLETENESS — reflect the WHOLE component:
  ...
  The loop feeds you a heuristic "untested variants/values/props" list AND each
  component's current coverage% — treat both as a STARTING point and also reason
  from the source (the percentage points at the lines still unhit). If a prop or
  variant is GENUINELY not observable under jsdom (purely visual, no
  role/text/state effect), leave a one-line `// covered:` or `// n/a:` comment in
  the test saying why, rather than silently skipping it.
...
THE FIX MANDATE (most important):
  ...
  when the test itself is wrong. Record every source fix in the journal and (if it
  needs follow-up) the recommendations backlog. Keep component fixes within the
  existing architecture (Box/Text + @knitui/core, theme-driven colour, hover/press/
  focusVisible states, a `disabled` variant) and the import boundary (no `tamagui`
  umbrella, no `@tamagui/*`, no `@mantine/*`).
...
MATCH WHAT EXISTS:
  ...
  a couple of the better-covered components before writing. Keep test style
  consistent across the kit; reuse shared helpers instead of re-deriving them.
PREAMBLE
    echo
    echo "REFERENCES (read before deepening a test; do NOT copy verbatim):"
    if [[ -n "${REF_COMPONENTS_DIR:-}" ]]; then
      echo "  - Mantine core source (the PUBLIC prop API + behaviour each test pins down):"
      echo "      $REF_COMPONENTS_DIR/<Name>"
    else
      echo "  - No local Mantine core reference found; assert against your knowledge of"
      echo "    the Mantine public API for each component."
    fi
    [[ -d "$CORE_REF" ]] && \
      echo "  - Our @knitui/core source (the design-system foundation: styled, theme, tokens): $CORE_REF"
    echo "  - Canonical example test to mirror: $PKG_DIR/src/Button/Button.test.tsx"
    echo "  - Shared render helper (reuse, do not replace): $PKG_DIR/src/test-utils.tsx"
  } > "$RULES_FILE"
}
...
reflect_plan_prompt() {  # $1=phase $2=remaining $3=open-recs $4=completeness report
  ...
  cat <<PROMPT
You are in a loop that deepens every component's tests in $PKG_NAME until most of
each component's features are covered (target: ${COVERAGE_TARGET}% statement
coverage per component). Read the rules file FIRST — it is the full spec and is
referenced, not repeated, every iteration:
...
Do TWO things, and ONLY these two (do NOT write tests in this step):
  1. REFLECT: append a short dated entry to $JOURNAL summarizing the PREVIOUS
     ...
     above — which tests got deepened, which are still thin (low coverage / many
  ...
  2. PLAN: (over)write $PLAN_FILE with a focused, single-iteration plan. In the
     "$phase" phase, weight it toward:
       - coverage phase → deepen tests for ~$BATCH_SIZE components from the
     ...
     specific variants/values/props/interactions the deepened test will exercise).
     Keep it to roughly one iteration of work.
PROMPT
}
...
For each component in the plan, write or deepen $PKG_DIR/src/<Name>/<Name>.test.tsx
so its assertions drive the component past ${COVERAGE_TARGET}% statement coverage,
exercising the WHOLE public surface (every variant value, every public prop,
controlled+uncontrolled value flow, callbacks, compound parts, ref forwarding) as
the rules describe. Reuse the shared render helper from "../test-utils"; import the
component from "./<Name>".

When a test exposes a component that is broken, incomplete, or divergent from the
Mantine public API, FIX THE COMPONENT SOURCE (within the existing architecture and
import boundary) rather than weakening the test — that is the fix mandate in the
rules. Record source fixes in $JOURNAL, and tick off any $RECS_FILE items you
...
Most components in $PKG_NAME are now at or above ${COVERAGE_TARGET}% coverage.
Read the rules file ($RULES_FILE) for context, then step back and review the
ENTIRE test suite under $PKG_DIR/src against the Mantine public API and against
...
Then one "- [ ] <actionable recommendation>" per line. Cover gaps such as:
  - components still below target or with shallow assertions (rendered but not
    behaviour-verified); untested variant values or boolean branches
  - controlled/uncontrolled flows, callbacks, or compound-part wiring not asserted;
    keyboard navigation and a11y roles/labels untested
  - component bugs the tests revealed but did not yet fix
...
component test suite (\`$TEST_CMD\`), AND a diff-scoped type-strictness check. Output
...
wrong (missing/divergent vs the Mantine public API), fix the component source — do
NOT weaken or delete the test to make it pass. Only adjust the test when the test
itself is wrong. You MUST NOT silence type errors with \`any\`, \`as any\`,
\`as unknown as\`, \`any[]\`, \`<any>\`, or @ts-ignore / @ts-nocheck /
...
# The gate: typecheck, then the component test suite (with coverage), then strict.
...
find_components_dir() {
  ...
  local canonical="$MANTINE_REF/src/components"
  if [[ -d "$canonical" ]]; then echo "$canonical"; return 0; fi
  local found
  found="$(find "$MANTINE_REF" -type d -path '*/src/components' 2>/dev/null | head -1)"
...
if _d="$(find_components_dir)" && [[ -n "$_d" ]] && \
  ...
  log "Using Mantine core reference: $REF_COMPONENTS_DIR"
else
  log "No Mantine core reference under $MANTINE_REF — tests assert from Mantine API knowledge."
fi
[[ -d "$CORE_REF" ]] && log "Foundation reference (@knitui/core): $CORE_REF"
...
log "Target: $total components in $PKG_NAME at >= ${COVERAGE_TARGET}% statements each."
...
# (which components are already well covered by their existing tests).
