import type { InputNativeCallbacks } from "./types";
...
export interface InputAndroidProps {
  ...
  /**
   * Padding between the inline image, when present, and the text input itself.
   * @platform android
   */
 ...
 */
export interface InputNativeProps extends InputIOSProps, InputAndroidProps, InputNativeCallbacks {
  /**
  ...
  // onEndEditing / onContentSizeChange / onScroll / onKeyPress are inherited from
  // InputNativeCallbacks (the unified InputCallbacks surface), so their payload
  // types live in one place alongside the cross-platform callbacks.
...
export type InputDataDetectorTypes =
  | "phoneNumber"
  | "link"
  | "address"
  | "calendarEvent"
...
export type InputReturnKeyType =
  // cross-platform
  | "done"
  | "go"
  | "next"
